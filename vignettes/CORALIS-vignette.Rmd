...
availsp() %>% 
  kbl(caption = "**Table 1:** Supported species in CORALIS for ncRNA-gene target enrichment analysis. Each entry indicates the type and number of ncRNA-target interaction in CORALIS by specie.", format = 'html', booktabs = TRUE) %>%
...
CORALIS calculates the enriched target genes from a list of ncRNAs provided by the user (input). CORALIS builds a 2x2 contingency table for each target gene and executes an hypergeometric distribution test, commonly known as one-tailed Fisher’s exact test. Table 2 shows the 2x2 contingency table.
...
df %>% 
    kbl(caption = "**Table 2:** The 2X2 contingenc table for gene i analysis as potential gene-target of ncRNAs that user set in the input with the cell fequencies represented as a, b, c and d, and the marginal totals as a+b, c+d, a+b, and c+b", format = "html") %>%
    kable_classic(full_width = F, html_font = "Cambria") 
...
Moreover, CORALIS provides other additional interesting statistical analyses such as  the odds ratio (OR), the relative risk (RR) and the adjusted p-value (false discovery rate (FDR), using Benjamin-Hochberg correction [2]):
 
$$OR=\frac{(\frac{a}{b})}{(\frac{c}{d})}$$

# 4. Installation and package loading 

Either the released or the development version of CORALIS can be installed from [GitHub](https://github.com/Daniel-VM/CORALIS) with:
...
library(CORALIS)
# browseVignettes("CORALIS") # access to this manual web page
```



# 5. ncRNA-target enrichment analysis with CORALIS
...
* The following arguments in *tienrich()* must be defined by the user: 
  ...
  + *fdr*: the desired adjusted p-value threshold at which the enriched target-genes are found. Default is 1, means all target genes will be displayed.
  + *organism*: character string specifying the scientific organism name (i.e: "*Homo sapiens*"). Run *availsp()* to list all available species.
  

## 5.1 CORALIS example - Human microRNA dataset

Let’s imagine that we are analyzing the expression profile of microRNAs (miRNAs), in a human specific cell type before and after certain treatment. Suppose that we obtain 9 out of 50 miRNAs with significantly higher expression after treatment. To analyze the potential biological effect of the 9 differentially expressed miRNAs and their connection into biological pathways, we use **CORALIS**.

Our aim here is to identify which genes can be potential enriched targets for these specific miRNAs. To do this we load an object containing 50 miRNAs IDs in ([miRBase id format](http://www.mirbase.org/search.shtml)). Among them, the 5 first items correspond to the deregulated miRNAs.
...
### 5.1.2 Visualization of enriched target genes

CORALIS also allows to visually analyze the output of the target enrichment analysis. Specifically, the *nodeVisu()* function which provides a series of graphs to explore the interaction between the ncRNAs of interest and their targeted genes, as well as different statistical parameters obtained in the enrichment analyses.

Firstly, you can use the ncRNA-target enrichment output directly as input of *nodeVisu()*, but it also supports the slot @results independently. The user can choose the number of *top* number of genes with higher significant inteactions with the ncRNA input.

With the *type* argument the graphic format is specified. 

* A) If *type* = "barplot" it displays a barplot summarizing the top targeted genes and the number of ncRNA interactions per bene. 
...
* B) If *type* = "network", then it creates an html widget to analyze interactively the network of interactions between the ncRNAs and their top target genes according to the enrichment analysis.

This graphical representation provides an interactive information on the interactions between ncRNAs and their top targeted genes. The nodes color of the 3D network graph represent all significant ncRNAs (miRNAs in this example) and their target genes in orange and purple, respectively. The larger the radious the larger the number of interactions between the enriched target genes and the ncRNAs. On the other hand, the links/edges thickness represent the FDR of the interactions (the thicker the link, the lower the FDR, and therefore, the signification of the interaction is higher).
...
* C) If *type* = "chord" display a chord plot to visualize the interactions between the input ncRNA and their top target genes (note: it allows a maximum of 25 gene targets).
...
Regarding the example used above, functional analysis revealed 9 biological processes significantly altered by the deferentially expressed miRNAs after the treatment. Specifically the miRNAs enriched target genes ABCC1 (entrezID:4363), GRM5 (entrezID:2915) and ICAM1 (entrezID:3383) play role in [GO:1904646](https://www.ebi.ac.uk/QuickGO/term/GO:1904646). 
...
# Find the miRNAs involved in the regulation of genes in GO:1904646
...
This information can be depicted a  network of interactions, where the genes within a specific GO term and their related miRNAs are represented.
