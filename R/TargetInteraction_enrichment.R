...
#' @param organism character string specifying the organism name (i.e: "Homo sapiens"). Run availsp() to see supported organisms and a summary of interactions by specie and ncRNA type.
#' @param min integer specifying the minimum number of interactions that target genes must have with the ncRNAs in the input in order to be analyzed. Default is min = 1.
...
#' Additionally, tienrich provides other interesting statistical parameters such the odds ratio (OR), the relative risk (RR) and the adjusted p-value (false discovery rate -FDR- using Benjamin-Hochberg correction).
#' @examples
#'# vector of ncRNA IDs
#'data(rnasID)
#'head(ids)
...
tienrich<-function(input_list, type, organism, min=1,  fdr=1){
  # Input processing
  info_args(a = input_list, b = type, c = organism)
  ...
  # Reformatting ncRNA IDs into SQLite vector format
  ...
  # 1.Evaluation of items in the input list
  nf <- info_items(k= my_mti$ncrnas, l=input_list)

  # 2. Evaluation of targets that are being processed
  list_of_genes <- info_targets(a = min , b = type, c = my_mti$genes)

  # Identify parameters for the hypergeometric distribution test and performs the statistics needed for overrepresentation/enrichment analysis
  do_enrichtment<-function(i){
    ...
    g  <- length(input_list)                        # Number of items in the input list
    N  <- total_mir                                 # Total number of ncRNAs with annotation in MIRTARBASE or RNAInter
    ...
    n  <- N-m                                       # Total number of ncRNAs (in the source databases) that interacts with other genes != gene "i"
